-- Expire old records where data has changed
...
-- Insert new rows for changed or new characters
MERGE INTO dim.dim_character d
USING staging.rick_and_morty_characters s
ON d.api_id = s.api_id AND d.is_current = true
WHEN NOT MATCHED THEN
  INSERT (api_id, character_name, episode_count, is_current, effective_date, created_at)
  VALUES (s.api_id, s.name, s.no_ep, true, CURRENT_DATE, s.created_at);